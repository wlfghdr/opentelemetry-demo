# Diff zum Upstream Repository

## Zusammenfassung

Dieser Fork (`wlfghdr/opentelemetry-demo`) wurde mit dem offiziellen Upstream-Repository (`open-telemetry/opentelemetry-demo`) verglichen.

### Status

- **Fork Repository:** wlfghdr/opentelemetry-demo
- **Upstream Repository:** open-telemetry/opentelemetry-demo
- **Vergleichsdatum:** 2025-12-20

### Commit-Übersicht

- **Commits im Fork (einzigartig):** 1
- **Commits hinter Upstream:** 1516

Der Fork ist erheblich hinter dem Upstream-Repository zurück (1516 Commits).
...
Die folgenden sind einige der neuesten Commits im Upstream, die nicht im Fork enthalten sind:

```
1c8e941 - build(deps): bump the pip-production-dependencies group across 2 directories with 2 updates
c8a9a91 - build(deps-dev): bump the npm-development-dependencies group across 1 directory with 5 updates
1412c8b - build(deps): bump github/codeql-action
be3030a - build(deps): bump the gradle-production-dependencies group across 2 directories with 3 updates
6474102 - build(deps): bump the mix-production-dependencies group across 1 directory with 2 updates
c82025c - build(deps): bump @bufbuild/protobuf
e9959d0 - build(deps): bump io.opentelemetry.instrumentation:opentelemetry-instrumentation-bom from 2.22.0 to 2.23.0
c601e5f - [chore] Bump dependent images
2d06de0 - [chore] update memory limits
24ebc07 - build(deps): bump actions/upload-artifact
34cd0ed - build(deps-dev): bump the npm-development-dependencies group across 2 directories with 3 updates
ccda75d - build(deps): bump the gradle-production-dependencies group across 2 directories with 1 update
d0c2b6f - build(deps): bump the go-production-dependencies group across 2 directories with 1 update
a0bdb54 - build(deps): bump github/codeql-action
1e3513b - build(deps): bump the pip-production-dependencies group across 4 directories with 11 updates
e046139 - build(deps): bump the npm-production-dependencies group across 2 directories with 4 updates
a7a5b64 - build(deps-dev): bump phoenix_live_reload in /src/flagd-ui
16530bd - build(deps): bump the mix-production-dependencies group across 1 directory with 2 updates
0ce0cab - build(deps-dev): bump the npm-development-dependencies group across 1 directory with 4 updates
ae2e225 - build(deps): bump the go-production-dependencies group across 2 directories with 13 updates
```
